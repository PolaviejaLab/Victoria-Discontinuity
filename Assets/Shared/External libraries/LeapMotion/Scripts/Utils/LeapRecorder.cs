...
public class LeapRecorder
{
	...
	/**
	 * Saves the recorded hand movements to a file 
	 */
	...
	{		
		...
		for (int i = 0; i < frames_.Count; ++i) {
			...
			Frame frame = new Frame();
			frame.DeserializeWithLength(frames_[i], frames_[i].Length);
			
			Debug.Log("Id: " + frame.Id + "; Timestamp: " + frame.Timestamp);
		}

		Debug.Log("Saved LEAP recording to " + path);
